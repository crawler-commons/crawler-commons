 ...
 *
 * @apiNote Creating a new {@link RobotsTagParser} instance incurs some overhead (normalizing user agents, compiling regular expressions).
...
public final class RobotsTagParser {
     ...
     */
    private final Pattern knownUserAgentsRegex;
    ...
    /**
     * @param targetUserAgents       Directives that apply to all user agents (e.g. {@code foo} in {@code X-Robots-Tag: foo}) are always collected by the parser, but directives from user agent groups (e.g. {@code bar} in {@code X-Robots-Tag: SomeBot: bar}) are only collected if the user agent (in this case {@code SomeBot}) is one of the target user agents.<br>
     ...
     */
    public RobotsTagParser(Set<String> targetUserAgents, Map<String, DirectiveParser<?>> directiveParsersByName, Consumer<ParserException> exceptionHandler) {
        ...
        this.knownUserAgentsRegex = ParserUtils.regexForCollectionElements(normalizedTargetUserAgents);
     ...
     */
    public RobotsTagParser(Set<String> targetUserAgents) {
     ...
     * <p>
     * A directive string is ambiguous if it contains at least one colon. In {@code X-Robots-Tag} headers, a colon indicates a user agent group or a key-value directive. There are three problems:
     * <ol>
     *     <li>Colons that separate user agents from directives are indistinguishable from colons that separate directive names from directive values.</li>
    ...
    private void parseAmbiguousString(String robotsHeader) {
        ...
        while (!stringToParse.isEmpty()) {
            ...
            if (delimiterIsColon && normalizedTargetUserAgents.contains(preprocessed.getFirstToken())) { //The first token is a target user agent.
                currentUserAgent = preprocessed.getFirstToken();
                ...
                DirectiveParser<?> parser = delimiterIsColon
                    ? directiveParsersByName.get(preprocessed.getFirstToken()) //If the first delimiter is a colon, then the first token is either a key-value directive or an unknown user agent.
                ...
                if (parser != null) {
                    try {
                        ...
                        if (currentUserAgent == null) {
                            ...
                            directiveCollection.addDirective(currentUserAgent, parserResult.getValue());
                        }
                        ...
                        stringToParse = preprocessed.getTail()
                            .map(tail -> {
                                ...
                                } else { //Either there is no next known directive name, or the next known directive name comes after a colon. That colon could indicate a user agent group, so ...
                                    return ParserUtils.dropUntilFirstMatch(knownUserAgentsRegex, preprocessed); //... skipping to the next known user agent is the only safe option.
                                }
                            })
                            .orElse("");
                    }
                } else { //The first token is either an unknown key-value directive name or an unknown user agent.
                    exceptionHandler.accept(new ParserException("Failed to parse unknown token \"" + preprocessed.getFirstToken() + "\". Is it a directive name or a user agent?"));
