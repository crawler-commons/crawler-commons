...
/**
 * Represents (part of) a string that is currently being processed by a {@link RobotsMetaParser} or a {@link RobotsTagParser}, along with additional information that is required by various parsing components.
 */
public final class PreprocessedString {
    ...
    /**
     * The index of the first delimiting character in the string, or {@code string.length()} if the string contains no delimiters.
     */
    private final int delimiterIndex;

    /**
     * The first delimiting character in the string. Can be a comma, a colon or {@link Optional#empty()}.
     */
    private final Optional<Character> delimiter;

    /**
     * Everything after the first delimiter (trimmed), or {@link Optional#empty()} if there is no delimiter or if there are no characters after the delimiter.
     */
    ...
    public PreprocessedString(String string) {
        ...
        String firstToken = string.substring(0, delimiterIndex)
            .trim()
    ...
    public boolean equals(Object object) {
        if (!(object instanceof PreprocessedString)) return false;
        PreprocessedString other = (PreprocessedString) object;

        return Objects.equals(string, other.string)
            && Objects.equals(firstToken, other.firstToken)
            && Objects.equals(delimiterIndex, other.delimiterIndex)
