...
/**
 * Parses the content of {@code <meta name="robots" content="...">} HTML
 * elements.
 * <p>
 * This parser is not thread-safe.
 * 
 * @apiNote Creating a new {@link RobotsMetaParser} instance incurs some
 *          overhead (normalizing product tokens, compiling regular
 *          expressions). It is therefore recommended to reuse existing parser
 *          instances (with {@link #reset()}) if possible.
 * @implNote There is no official standard or specification for
 *           {@code <meta name="robots">} elements. In some cases, their syntax
 *           is ambiguous, which makes parsing difficult. Different vendors may
 *           define and support different directives.
 * @see <a
 *      href="https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/meta/name/robots">MDN:
 *      &lt;meta name=&quot;robots&quot;&gt;</a>
 * @see <a
...
public final class RobotsMetaParser {
     ...
     *            {@code <meta name="robots" content="foo">}) are always
     *            collected by the parser, but directives that only apply to
     *            specific robots (e.g. {@code bar} in
     *            {@code <meta name="SomeBot" content="bar">}) are only
     ...
     * <p>
     * If the element has a {@code name} and a {@code content} attribute and if
     * the value of the {@code name} attribute is equal to "robots" or one of
     * the target product tokens, then the directives from the {@code content}
     * attribute are parsed and collected.
     * <p>
     * This method can handle empty strings, empty {@code <meta>} elements,
     * {@code <meta>} elements that lack the required attributes, and
     * {@code <meta>} elements that have unrelated attributes.
     * <p>
     ...
     * {@code <meta name="robots">} elements, a colon indicates a key-value
     * directive. There are two problems:
     * <ol>
        ...
        while (!stringToParse.isEmpty()) {
            ...
            if (parser != null) {
                try {
                    ...
                    // Remove the parsed directive (including its value, if
                    // applicable) from the string:
